...
def notify(title, msg, duration=4):
    try:
...
# ---------------------------
# NT TIMER RESOLUTION (NEW)
# ---------------------------
ntdll = ctypes.WinDLL("ntdll")

NtQueryTimerResolution = ntdll.NtQueryTimerResolution
NtQueryTimerResolution.argtypes = [
    ctypes.POINTER(ctypes.c_ulong),
    ctypes.POINTER(ctypes.c_ulong),
    ctypes.POINTER(ctypes.c_ulong)
]
NtQueryTimerResolution.restype = ctypes.c_ulong
...
NtSetTimerResolution.restype = ctypes.c_ulong
...
def set_timer_resolution_100ns(value):
    current = ctypes.c_ulong()
    status = NtSetTimerResolution(value, True, ctypes.byref(current))
    if status == 0:
        _timer_state["active"] = True
        _timer_state["requested"] = value
        return True, current.value
    return False, None

def reset_timer_resolution():
    if not _timer_state["active"]:
        return
    current = ctypes.c_ulong()
    NtSetTimerResolution(_timer_state["requested"], False, ctypes.byref(current))
    _timer_state["active"] = False
    timeEndPeriod(1)
...
class MainWindow(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.rb_timer_05.setChecked(True)
        ...
        self.rb_freq_1m = QtWidgets.QRadioButton("1 Min")
        self.rb_freq_5m = QtWidgets.QRadioButton("5 Min")
        self.rb_freq_10m = QtWidgets.QRadioButton("10 Min")
        self.rb_freq_5m.setChecked(True)
        fg_layout.addWidget(self.rb_freq_30s)
        fg_layout.addWidget(self.rb_freq_1m)
        fg_layout.addWidget(self.rb_freq_5m)
        fg_layout.addWidget(self.rb_freq_10m)
    ...
    def update_frequency_state(self):
        # Disable frequency options if Default is selected
        disable = self.rb_timer_default.isChecked()
        for rb in [self.rb_freq_30s, self.rb_freq_1m, self.rb_freq_5m, self.rb_freq_10m]:
        ...
        def worker():
            try:
                ...
                if self.rb_timer_05.isChecked():
                    ok, _ = set_timer_resolution_100ns(5000)  # 0.5 ms
                    notify(APP_NAME, "Timer resolution set to 0.5 ms")
                elif self.rb_timer_1.isChecked():
                    ok, _ = set_timer_resolution_100ns(10000)  # 1 ms
                    if not ok:
                        timeBeginPeriod(1)  # fallback
                    notify(APP_NAME, "Timer resolution set to 1 ms")
                else:
                ...
                # frequency
                if self.rb_freq_30s.isChecked():
                    freq = 30
                elif self.rb_freq_1m.isChecked():
                    freq = 60
                elif self.rb_freq_5m.isChecked():
                    freq = 300
                elif self.rb_freq_10m.isChecked():
                    freq = 600
                else:
                    freq = 300
